...
void clConnector::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    if (this->bUp_Down)
    {
        painter->drawLine(QLine(0,0,iDisX/2,0));                                    //First horisontal part equal to half the horisontal distance
        painter->drawLine(QLine(iDisX/2,0,iDisX/2,iDisY));                          //If the bUp_Down Flag is true draw the line upwards
        painter->drawLine(QLine(iDisX/2,iDisY,iDisX,iDisY));                        //Complete the second part of the horisontal distance
    }
    else
    {
